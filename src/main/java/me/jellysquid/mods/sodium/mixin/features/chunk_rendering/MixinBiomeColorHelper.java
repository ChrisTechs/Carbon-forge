...
public class MixinBiomeColorHelper {
    ...
    private static int getColorAtPos(IBlockAccess blockAccess, BlockPos pos, BiomeColorHelper.ColorResolver colorResolver)
    {
        ...
        if (radius == 0) {
            return ReflectionHelper.getColorAtPosMethod(colorResolver, blockAccess.getBiomeGenForCoords(pos), pos);
        } else {
            ...
            for(int z = -radius; z <= radius; z++) {
                for(int x = -radius; x <= radius; x++) {
                    ...
                    int l = ReflectionHelper.getColorAtPosMethod(colorResolver, blockAccess.getBiomeGenForCoords(mutablePos), mutablePos);
                    i += (l & 16711680) >> 16;
                    j += (l & 65280) >> 8;
                    k += l & 255;
                }
