    ...
    public ChunkBuildResult<T> performBuild(ChunkRenderCacheLocal cache, ChunkBuildBuffers buffers, CancellationSource cancellationSource) {
        ...
        if(!data.isEmpty()) {
            ...
            for(BlockRenderPass pass : TRANSLUCENT_PASSES) {
                ...
                ChunkBufferSorter.sortStandardFormat(buffers.getVertexType(), sortedData, sortedData.capacity(), (float) camera.xCoord - offset.getX(), (float)camera.yCoord - offset.getY(), (float)camera.zCoord - offset.getZ());
